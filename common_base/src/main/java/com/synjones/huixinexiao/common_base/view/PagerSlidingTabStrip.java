...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int tabBackgroundResId = R.drawable.viewpage_tab_bg;

	private Locale locale;

	private int selectTitleColor = getResources().getColor(R.color.white);
	...
	private int normalTitleColor = getResources().getColor(R.color.gray1);
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		// tabTextSize = (int)
		// TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, tabTextSize,
		// dm);
		...
		selectTitleColor = a.getColor(
				R.styleable.PagerSlidingTabStrip_pstSelectedTabTextColor,
				selectTitleColor);
		normalTitleColor = a.getColor(
				R.styleable.PagerSlidingTabStrip_pstNormalTabTextColor,
				normalTitleColor);
		tabTextSize = a.getDimensionPixelSize(
				R.styleable.PagerSlidingTabStrip_pstsIndicatorTextSize,
				tabTextSize);
		a.recycle();
	...
	private void addTab(final int position, View tab) {
		...
		if (tab instanceof TextView) {
			...
			if (position == 1) {
				tv.setTextColor(selectTitleColor);
			} else {
				tv.setTextColor(normalTitleColor);
			}
		}
			...
			if (v instanceof LinearLayout) {
				...
				if (0 != linearLayout.getChildCount()) {
					...
					if (linearLayout.getChildAt(0) instanceof TextView) {
						...
						TextView tab = (TextView) linearLayout.getChildAt(0);
						tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
						// tab.setTypeface(tabTypeface, tabTypefaceStyle);
						if (i == 0) {
							tab.setTextColor(selectTitleColor);
						} else {
							tab.setTextColor(normalTitleColor);
						}
	...
	protected void onDraw(Canvas canvas) {
		...
		float off = 0.0f;
		float halfWid = 0.0f;
		if (currentTab instanceof LinearLayout) {
			LinearLayout linearLayout = (LinearLayout) currentTab;
			if (0 != linearLayout.getChildCount()) {
				if (linearLayout.getChildAt(0) instanceof TextView) {
					if (linearLayout.getChildAt(0) instanceof TextView) {// edit
																			// by
																			// leip
						TextView tv = (TextView) linearLayout.getChildAt(0);
						rectPaint.setTextSize(tv.getTextSize());
						off = rectPaint.measureText(tv.getText().toString()) / 2;
						halfWid = (lineRight - lineLeft) / 2;
					}
				}
			}
		}
		...
		canvas.drawRect((lineLeft + halfWid) - off, height - indicatorHeight,
				(lineRight - halfWid) + off, height, rectPaint);
		// draw underline
			...
			for (int i = 0; i < tabCount; i++) {
				...
				if (view instanceof ImageButton) {
					...
					if (viewText instanceof TextView) {
						if (position == i) {
							tabTextView.setTextColor(getResources().getColor(
									R.color.white));
						} else {
							tabTextView.setTextColor(getResources().getColor(
									R.color.gray1));
